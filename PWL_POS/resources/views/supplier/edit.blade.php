    ...
    <div class="card-body">
        <form method="POST" action="{{ url('supplier/'. $supplier->supplier_id) }}">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="supplier_kontak" name="supplier_kontak" value="{{ $supplier->supplier_kontak }}">
            </div>
            <div class="form-group">
                <label for="supplier_alamat">Alamat Supplier</label>
                <textarea class="form-control" id="supplier_alamat" name="supplier_alamat">{{ $supplier->supplier_alamat }}</textarea>
            </div>
            <button type="submit" class="btn btn-success">Simpan</button>
            <a href="{{ url('supplier.index') }}" class="btn btn-warning">Kembali</a>
        </form>
    </div>
</div>
@endsection